...
class AirService_cell:
   ...
   # %%
    ...
    def train(self,epoch):
        ...
        q_win_list_t=q_list_t[:,len(X_CUR[0]):(len(X_CUR[0])+len(X_WIN[0]))]
        q_ap_list_t=q_list_t[:,(len(X_CUR[0])+len(X_WIN[0])):(len(X_CUR[0])+len(X_AP[0]))+len(X_WIN[0])]
        q_t_list_t=q_list_t[:,(len(X_CUR[0])+len(X_AP[0]))+len(X_WIN[0]):(len(X_CUR[0])+len(X_AP[0])+len(X_WIN[0])+len(X_T[0]))]
        ...
        q_win_list_t_plus_1=q_list_t_plus_1[:,len(X_CUR[0]):(len(X_CUR[0])+len(X_WIN[0]))]
        q_ap_list_t_plus_1=q_list_t_plus_1[:,(len(X_CUR[0])+len(X_WIN[0])):(len(X_CUR[0])+len(X_AP[0]))+len(X_WIN[0])]
        ...
        for index, (states_t,actions,reward_t,states_t_plus_1) in enumerate(minibatch):
            ...
            if index==0:
                ...
                Y=torch.cat((q_cur_t,q_win_t,q_ap_t, q_t_t, q_et_t),axis=1)
                
            else:
                X=torch.cat((X,states_t),axis=0)
                q_t=torch.cat((q_cur_t,q_win_t,q_ap_t, q_t_t, q_et_t),axis=1)
...
#         torch.save(airService.airServiceModel.state_dict(),f'data/lstm/airService_air_lstm_v1_{1+int(epoch/200)}_sig_range_8_v1_noeco.pth')
#         np.save(f'data/lstm/totalEpochRewards_air_lstm_v1_{1+int(epoch/200)}_sig_range_8_v1_noeco.npy',totalEpochRewards)


# torch.save(airService.airServiceModel.state_dict(),'data/lstm/airService_air_lstm_v1_sig_range_8_v1_noeco.pth')
# np.save(f'data/lstm/totalEpochRewards_air_lstm_v1_sig_range_8_v1_noeco.npy',totalEpochRewards)
