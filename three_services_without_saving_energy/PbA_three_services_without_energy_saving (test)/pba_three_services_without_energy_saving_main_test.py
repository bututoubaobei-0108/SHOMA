# define the stateless lstm for priorities calculators
# %%
import torch 
import numpy as np
import pandas as pd
...
from PRIORITIES_PBSHOMMA_TEMP_AIR_CELL import *
...
lightService.lightServiceModel.load_state_dict(torch.load('./../../data/lightService_lstm_v1_25_mlt_pbshomma_noeco_even.pth'))
lightService.targetLightServiceModel.load_state_dict(torch.load('./../../data/lightService_lstm_v1_25_mlt_pbshomma_noeco_even.pth'))

tempService=TempService_cell()
tempService.tempServiceModel.load_state_dict(torch.load('./../../data/tempService_lstm_v1_25_mlt_pbshomma_noeco_even.pth'))
tempService.targetTempServiceModel.load_state_dict(torch.load('./../../data/tempService_lstm_v1_25_mlt_pbshomma_noeco_even.pth'))

airService=AirService_cell()
airService.airServiceModel.load_state_dict(torch.load('./../../data/airService_lstm_v1_25_mlt_pbshomma_noeco_even.pth'))
airService.targetAirServiceModel.load_state_dict(torch.load('./../../data/airService_lstm_v1_25_mlt_pbshomma_noeco_even.pth'))

cur_calculator=CalculationPriorities_cell(num_inputs=X_US.shape[1]+1+1+1,num_outputs=3)
cur_calculator.calPrioModel.load_state_dict(torch.load('./../../data/cur_calculator_v1_25_mlt_pbshomma_noeco_even.pth'))
cur_calculator.targetCalPrioModel.load_state_dict(torch.load('./../../data/cur_calculator_v1_25_mlt_pbshomma_noeco_even.pth'))

win_calculator=CalculationPriorities_cell(num_inputs=X_US.shape[1]+1+1,num_outputs=2)
win_calculator.calPrioModel.load_state_dict(torch.load('./../../data/win_calculator_v1_25_mlt_pbshomma_noeco_even.pth'))
win_calculator.targetCalPrioModel.load_state_dict(torch.load('./../../data/win_calculator_v1_25_mlt_pbshomma_noeco_even.pth'))

t_calculator=CalculationPriorities_cell(num_inputs=X_US.shape[1]+1+1,num_outputs=2)
t_calculator.calPrioModel.load_state_dict(torch.load('./../../data/t_calculator_v1_25_mlt_pbshomma_noeco_even.pth'))
t_calculator.targetCalPrioModel.load_state_dict(torch.load('./../../data/t_calculator_v1_25_mlt_pbshomma_noeco_even.pth'))


X_US_2=torch.from_numpy(np.load(f'./../../data/X_US_lstm_v1_eishomma_even_100.npy',allow_pickle=True))
X_LE_2=torch.from_numpy(np.load(f'./../../data/X_LE_lstm_v1_eishomma_even_100.npy',allow_pickle=True))
X_TE_2=torch.from_numpy(np.load(f'./../../data/X_TE_lstm_v1_eishomma_even_100.npy',allow_pickle=True))
X_AE_2=torch.from_numpy(np.load(f'./../../data/X_AE_lstm_v1_eishomma_even_100.npy',allow_pickle=True))
...
# %%  
    ...
    for step in range(0,steps):
        ...
        x_t_p_t_norm=torch.cat((x_us_t_norm,x_te_t_norm,x_ae_t_norm),axis=1)
        ...
        if sigma>0.1:
            ...
            win_temp_p_t_norm=win_temp_p_t/(torch.abs(win_temp_p_t).sum())
            # win_temp_p_t_norm=torch.tensor([[1]], dtype=torch.float32, device=device)
            win_air_p_t=win_priorities[0,[1]]
            win_air_p_t_norm=win_air_p_t/(torch.abs(win_air_p_t).sum())
            # win_air_p_t_norm=torch.tensor([[1]], dtype=torch.float32, device=device)

            # window and curtain open time
            t_priorities=t_calculator.getPriorities(x_t_p_t_norm)
            t_temp_p_t=t_priorities[0,[0]]
            t_temp_p_t_norm=t_temp_p_t/(torch.abs(t_temp_p_t).sum())

            t_air_p_t=win_priorities[0,[1]]
            t_air_p_t_norm=t_air_p_t/(torch.abs(t_air_p_t).sum())
            ...
            Q_cur_t_norm=cur_light_p_t*Q_light_cur_t_norm+cur_temp_p_t_norm*Q_temp_cur_t_norm+cur_air_p_t_norm*Q_air_cur_t_norm
        ...
        x_t_p_t_new_norm=torch.cat((x_us_t_new_norm,x_te_t_new_norm,x_ae_t_new_norm),axis=1)
        ...
        t_transition=(x_win_p_t_norm,win_priorities_actions,win_priorities_reward,x_win_p_t_new_norm)
        ...
        t_calculator.updateReplayMemory(t_transition)
...
#         cur_calculator.targetCalPrioModel.load_state_dict(cur_calculator.calPrioModel.state_dict())
#         win_calculator.targetCalPrioModel.load_state_dict(win_calculator.calPrioModel.state_dict())
#         t_calculator.targetCalPrioModel.load_state_dict(t_calculator.calPrioModel.state_dict())
             
#     if epoch%50==0 and epoch!=0:      
#         torch.save(lightService.lightServiceModel.state_dict(),f'data/lstm/lightService_lstm_v1_{1+int(epoch/50)}_mlt_pbshomma_noeco_even.pth')
#         torch.save(tempService.tempServiceModel.state_dict(),f'data/lstm/tempService_lstm_v1_{1+int(epoch/50)}_mlt_pbshomma_noeco_even.pth')
#         torch.save(airService.airServiceModel.state_dict(),f'data/lstm/airService_lstm_v1_{1+int(epoch/50)}_mlt_pbshomma_noeco_even.pth')

#         torch.save(cur_calculator.calPrioModel.state_dict(),f'data/lstm/cur_calculator_v1_{1+int(epoch/50)}_mlt_pbshomma_noeco_even.pth')
#         torch.save(win_calculator.calPrioModel.state_dict(),f'data/lstm/win_calculator_v1_{1+int(epoch/50)}_mlt_pbshomma_noeco_even.pth')
#         torch.save(t_calculator.calPrioModel.state_dict(),f'data/lstm/t_calculator_v1_{1+int(epoch/50)}_mlt_pbshomma_noeco_even.pth')

#         np.save(f'data/lstm/totalEpochRewards_lstm_v1_{1+int(epoch/50)}_mlt_pbshomma_noeco_even.npy',totalEpochRewards)
#         np.save(f'data/lstm/totalAcc_lstm_v1_{1+int(epoch/50)}_mlt_pbshomma_noeco_even.npy',totalAcc)
        

# torch.save(lightService.lightServiceModel.state_dict(),'data/lstm/lightService_lstm_v1_mlt_pbshomma_noeco_even.pth')
# torch.save(tempService.tempServiceModel.state_dict(),'data/lstm/tempService_lstm_v1_mlt_pbshomma_noeco_even.pth')
# torch.save(airService.airServiceModel.state_dict(),'data/lstm/airService_lstm_v1_mlt_pbshomma_noeco_even.pth')
# torch.save(cur_calculator.calPrioModel.state_dict(),'data/lstm/cur_calculator_v1_mlt_pbshomma_noeco_even.pth')
# torch.save(win_calculator.calPrioModel.state_dict(),'data/lstm/win_calculator_v1_mlt_pbshomma_noeco_even.pth')
# torch.save(t_calculator.calPrioModel.state_dict(),'data/lstm/t_calculator_v1_mlt_pbshomma_noeco_even.pth')

# np.save(f'data/lstm/totalEpochRewards_lstm_v1_mlt_pbshomma_noeco_even.npy',totalEpochRewards)
# np.save(f'data/lstm/totalAcc_lstm_v1_mlt_pbshomma_noeco_even.npy',totalAcc)

        
print(f'100 acc: {np.average(totalAcc,axis=0)}')

np.save(f'./../../data/totalAcc_lstm_v1_mlt_pbshomma_noeco_even_100.npy',totalAcc)
np.save(f'./../../data/replayMemory_light_lstm_v1_mlt_pbshomma_noeco_even_100.npy',lightService.replayMemory)
np.save(f'./../../data/replayMemory_temp_lstm_v1_mlt_pbshomma_noeco_even_100.npy',tempService.replayMemory)
np.save(f'./../../data/replayMemory_air_lstm_v1_mlt_pbshomma_noeco_even_100.npy',airService.replayMemory)
