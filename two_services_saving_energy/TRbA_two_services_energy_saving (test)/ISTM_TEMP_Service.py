...
class LstmServiceModel(torch.nn.Module):
    def __init__(self,hidden_layers=40):
...
class TempService_cell:
    ...
    def getReward(self):
        ...
        if self.x_us_t==torch.tensor([[0]],dtype=torch.float32,device=device): 
            if self.x_ac_t==torch.tensor([[0]],dtype=torch.float32,device=device) and self.x_win_t==torch.tensor([[0]],dtype=torch.float32,device=device) and self.x_cur_t==torch.tensor([[0]],dtype=torch.float32,device=device):
                self.reward=4
            else:
                self.reward=-4
                        ...
                        for j in range(X_WIN.shape[1]):
                            ...
                            for k in range(X_T.shape[1]):
                                for k2 in range(X_ET.shape[1]):
                                    ...
                                    x_tr_new_t_int=self.getIndoorTemp(self.x_tr_t,x_ac_new_t_int,x_cur_new_t_int,x_win_new_t_int,x_t_new_t_int, x_et_new_t_int)
        ...
        if self.x_us_t==torch.tensor([[3]],dtype=torch.float32,device=device): 
            if self.x_tr_t>=torch.tensor([[17]],dtype=torch.float32,device=device) and self.x_tr_t<=torch.tensor([[20]],dtype=torch.float32,device=device):
                if self.x_ac_t==torch.tensor([[0]],dtype=torch.float32,device=device):
                    self.reward=8
                else:
                    x_ac_new_t_int=torch.tensor([[0]],dtype=torch.float32,device=device)
                        ...
                        for j in range(X_WIN.shape[1]):
                            ...
                            for k in range(X_T.shape[1]):
                                for k2 in range(X_ET.shape[1]):
                                    ...
                                    x_et_new_t_int=X_ET[[[0]],k2]
                                    x_tr_new_t_int=self.getIndoorTemp(self.x_tr_t,x_ac_new_t_int,x_cur_new_t_int,x_win_new_t_int,x_t_new_t_int,x_et_new_t_int)
                                    if x_tr_new_t_int>=torch.tensor([[17]],dtype=torch.float32,device=device) and x_tr_new_t_int<=torch.tensor([[20]],dtype=torch.float32,device=device):
                                        self.reward=-8
                                        stop=True
                                        break
                                    else:
