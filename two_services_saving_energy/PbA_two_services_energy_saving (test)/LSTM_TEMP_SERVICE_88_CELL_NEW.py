                            ...
                            for k in range(X_T.shape[1]):
                                ...
                                for k2 in range(X_ET.shape[1]):
                                    ...
                                    if x_tr_new_t_int>=torch.tensor([[23]],dtype=torch.float32,device=device) and x_tr_new_t_int<=torch.tensor([[25]],dtype=torch.float32,device=device):
                                        # because the choice of electric device can be caused by the other services. thus, xxx_CELL_NEW is specifically for multi-service structures with new reward functions
                                        self.reward=2
                                        stop=True
                                        break
                                    else:
...
#         torch.save(tempService.tempServiceModel.state_dict(),f'data/lstm/tempService_lstm_v1_{1+int(epoch/200)}_sig_88.pth')

#         np.save(f'data/lstm/totalEpochRewards_temp_lstm_v1_{1+int(epoch/200)}_sig_88.npy',totalEpochRewards)


# torch.save(tempService.tempServiceModel.state_dict(),'data/lstm/tempService_lstm_v1_sig_88.pth')
# np.save(f'data/lstm/totalEpochRewards_temp_lstm_v1_sig_88.npy',totalEpochRewards)
